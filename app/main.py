...
async def lifespan(app: FastAPI):
    """Preload MiniLM model on startup."""
    print("[startup] Preloading MiniLM model...")
    try:
        from app.ingestion.text_embedder import _get_model
        loop = asyncio.get_event_loop()
        await loop.run_in_executor(None, _get_model)
        print("[startup] MiniLM ready")
    except Exception as e:
        print(f"[startup] MiniLM preload failed (will load on first request): {e}")
